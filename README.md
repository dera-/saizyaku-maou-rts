# 触媒モンスター ~最弱魔王の防衛戦~

プレイヤー自身が「移動する本拠地」となる、ニコ生ゲーム向けランキング型タワーディフェンスRTSです。

## 操作

- 開始画面: 難易度を選び「○○で開始」を押す（初期選択はノーマル）。押さなくても10秒後に選択中の難易度で自動開始
- 魔王をドラッグ: 魔王が指・カーソルを追従して移動
- 右下のバーチャルパッド: 通常時は「選択解除」ボタンの位置に表示され、押している方向へ魔王を移動。召喚地点選択中はパッドが「選択解除」ボタンに切り替わる
- 触媒を押す: 召喚に使う触媒を最大3個まで選択
- 触媒選択後に戦場をタップ: タップ地点へモンスターを召喚
- 選択解除: 現在の召喚レシピをクリア
- 召喚または選択解除時は魔王の移動予約を破棄し、ドラッグ入力が残りません。

## ゲームルール

- ノーマルは従来どおりの標準難易度です。魔王は5秒間被弾しなければ毎秒4HP自然回復します。
- ハードは魔王の自然回復がなく、敵HPが30%、攻撃力が25%、移動速度が8%上昇し、攻撃間隔が10%短縮されます。敵撃破・勇者早期撃破・ノーデスなど、すべてのプラス得点は5倍です。死亡減点はノーマルと共通です。
- ハードは通常敵の出現間隔がさらに18%短く、追加出現率が10ポイント上昇し、同時出現上限が42体から48体になります。
- ハードの敵は1触媒の低位召喚を優先して狙い、掃討戦士・粉砕騎士・勇者は密集したモンスターへ範囲攻撃します。突破盗賊と勇者の魔王襲撃は、魔王付近の護衛型モンスターで阻止できます。
- ハードでは2～3触媒を種族の適性地形へ召喚すると「地形共鳴」が発生し、HP・攻撃・攻撃速度・被ダメージが有利になります。適性は骨族＝墓地、魔獣族＝街道、鉱石族＝岩場、精霊族＝魔力溜まり、粘体族＝毒沼、魔族＝墓地です。
- プレイ時間は140秒、残り20秒（開始120秒）で勇者が出現します。
- 魔王は50HPを持ち、被弾後0.45秒は連続ダメージを受けません。
- HPが0になると死亡し、3秒後に死亡地点で全回復して復活します。
- 触媒選択中は最大5秒間、敵と味方と敵出現が停止します。
- 魔王死亡中は敵が攻撃を中止して復活地点から離れ、復活後は2秒間無敵になります。
- 魔王死亡中は全敵が個別の方向へ強制散開し、復活地点に留まりません。
- 遠距離攻撃は画面上を飛ぶ弾、近距離攻撃は斬撃線とダメージ数値で表示されます。
- 画面左右の勢力パネルで敵編成と、味方の名前・合計HP・種族・戦術を確認できます。
- 触媒未選択時は触媒欄、選択中は召喚可能な戦場全体が点滅し、操作手順を `①` `②` で案内します。
- 軍勢コスト上限は35秒ごとに `6 → 9 → 12 → 15` と拡張されます。
- 残りコストでは召喚できない触媒は暗くなり「上限」と表示されますが、所持数の `×個数` は常に確認できます。
- 街道・岩場・魔力溜まり・毒沼・墓地には召喚効果を常時表示します。
- 魔王死亡中は味方の攻撃力が30%低下します。
- 魔王と収集型モンスターはフィールド上の触媒を自動回収します。
- 各触媒は最大99個まで所持できます。
- 敵撃破、連続撃破、魔王近距離での危険撃破、勇者早期撃破、ノーデスで得点します。
- 魔王死亡時は `1500 + それまでの死亡回数 × 500` 点を失います。

## 召喚

基本触媒は骨・牙・鉄・魔力・毒茸・魂火の6種類です。最初の触媒が基本種族、2～3個目が能力と戦術を決めます。召喚地点の街道・岩場・魔力溜まり・毒沼・墓地でも性能が変化します。35秒ごとに魔王軍が `×0.88 → ×1.15 → ×1.50 → ×2.00` と段階強化され、新規召喚だけでなく既存モンスターのHP・攻撃力・スプライトにも反映されます。中盤は通常出現間隔を短縮し、35秒以降は4～16%の確率で敵が追加出現します。終盤の敵能力倍率は最大 `×2.15`、通常出現間隔は最短2.45秒です。

代表例:

- 骨 + 鉄: アーマースケルトン
- 骨 + 魔力: スケルトンメイジ
- 牙 + 毒茸: ヴェノムハウンド
- 鉄 + 魂火: インフェルノゴーレム
- 魔力 + 魂火: レイス

## 画像素材

- `assets/sprites.png`: 魔王、味方モンスター、敵冒険者、勇者、触媒アイコンを収録した6×4の透過スプライトアトラス
- `assets/field-game.png`: 岩場・魔力溜まり・街道・墓地・毒沼を描いた1280×720のフィールド背景

## モバイル対応

- バーチャルパッドを離した瞬間に入力ベクトルと移動目標を現在位置へリセットし、意図しない戻り移動を防止
- iOS・低性能端末の初期ロードを止めないよう、BGM・通常SE・終盤SEを段階的に非同期ロードし、長尺BGMはストリーミング再生
- Android向けにWebGL描画を優先し、UI・HPバー・ドロップの差分描画、距離計算、同時エフェクト数を最適化
- 低性能端末向けに空ラベルEntityを廃止し、敵・弾・エフェクト数を制限。混雑時はユニットAIを隔フレーム更新して時間進行と操作応答を維持

画像はゲーム専用に生成したオリジナルの8-bit家庭用RPG風ピクセルアートです。スプライトはクロマキー除去後の透過PNGを使用し、Akashic Engine上ではアトラスの各セルを切り出して表示します。

## フォント素材

- 使用フォント: PixelMplus12 Regular / PixelMplus12 Bold
- 作者: itouhiro（M+ BITMAP FONTSをTrueType化）
- ライセンス: M+ FONT LICENSE（個人・商用を問わず利用、改変、再配布可能）
- 配布元: https://github.com/itouhiro/PixelMplus
- ライセンス原文: `assets/font/LICENSE_E.txt`

ゲームのファミコン風RPG画面に合わせ、全ラベルをPixelMplus12のBitmapFontへ統一しています。`tools/generate-font-chars.js` でゲーム内の使用文字を `assets/font/chars.txt` に抽出し、`@akashic/bmpfont-generator` の `bmpfont-generator` コマンドで18px・24px・28pxの通常書体と36px・48pxの太字書体をPNG・JSONへ変換しています。スマートフォンで縮小表示した際に文字の細い画素が欠けないよう、アンチエイリアスを有効にしています。

配布データを軽量化するため、ゲーム実行に必要なPNG・JSONとライセンス文書のみ同梱しています。フォントを再生成する場合は、上記配布元から `PixelMplus12-Regular.ttf` / `PixelMplus12-Bold.ttf` を `assets/font` へ取得してから生成してください。`chars.txt` は生成スクリプト実行時に作成されます。

生成例:

```powershell
node tools/generate-font-chars.js
bmpfont-generator assets/font/PixelMplus12-Regular.ttf assets/font/pixelmplus-24.png -H 24 -f assets/font/chars.txt -m "？" -F "#ffffff" --json assets/font/pixelmplus-24.json --margin 1
```

## BGM・効果音素材

音楽・効果音：魔王魂（作曲：森田交一）、効果音ラボ

Akashic Engineの対応環境に合わせ、各素材のOGG版とM4A版を `assets/audio` に収録しています。素材の取得元として、各素材を確認できる公式配布ページURLを記載します。

| 用途 | 使用素材 | 公式配布ページ |
| --- | --- | --- |
| ゲームBGM（開始画面～ゲーム終了） | 8bit07「けっせん」ループ版 | https://maou.audio/bgm_8bit07/ |
| アイテム取得SE | レトロ03 | https://maou.audio/se_8bit03/ |
| モンスター召喚SE | 魔法陣を展開 | https://soundeffect-lab.info/sound/battle/ |
| モンスター死亡SE | レトロ12 | https://maou.audio/se_8bit12/ |
| 魔王被ダメージSE | レトロ07 | https://maou.audio/se_8bit07/ |
| 魔王死亡SE | レトロ06 | https://maou.audio/se_8bit06/ |
| 敵撃破SE | レトロ05 | https://maou.audio/se_8bit05/ |
| 勇者出現SE | 文字表示の衝撃音2 | https://soundeffect-lab.info/sound/anime/ |
| 勇者撃破SE | ジングル02 | https://maou.audio/game_jingle02/ |
| ゲーム終了時SE | ジングル03 | https://maou.audio/game_jingle03/ |

- 魔王魂: https://maou.audio/
- 音楽利用のルール: https://maou.audio/rule/
- 効果音ラボ: https://soundeffect-lab.info/
- 効果音ラボ利用規約: https://soundeffect-lab.info/agreement/

## 開発

```powershell
npm install
npm test
npm run build
npm start
```

`npm start` で Akashic Sandbox を起動します。ランキングスコアは `g.game.requestSaveScore()` へ送信されます。ランキング期間中の公平性確保のため、ニコ生の `randomSeed` が与えられた場合は `g.XorshiftRandomGenerator` を使用します。

## 主なファイル

- `script/main.js`: UI、入力、リアルタイム戦闘、進行、ランキングスコア送信
- `script/gameLogic.js`: 召喚レシピ、能力補正、敵強化、スコア計算
- `script/_bootstrap.js`: ニコ生セッションパラメーターと乱数シード受け取り
- `test/gameLogic.test.js`: 純粋ロジックの自動テスト
- `assets/sprites.png`: キャラクター・モンスター・敵・触媒の画像アトラス
- `assets/field-game.png`: 8-bit風フィールド背景
- `assets/audio/`: 魔王魂・効果音ラボのBGM／効果音（OGG/M4A）
